...
public class GestorPreguntas : MonoBehaviour
{
    ...
    {
        public Pregunta[] básico;
        public Pregunta[] intermedio;
        public Pregunta[] avanzado;
    }
    ...
    void CargarPreguntasJson()
    {
        TextAsset archivoJSON = Resources.Load<TextAsset>("JsonPreguntasJuegos/Metales Alcalinos (Grupo 1)");

        if (archivoJSON != null)
        {
            PreguntasNivel datos = JsonUtility.FromJson<PreguntasNivel>(archivoJSON.text);

            // Mapeo de niveles (básico=1, intermedio=4, avanzado=7)
            preguntasPorNivel[0] = ConvertirPreguntas(datos.niveles.básico);
            preguntasPorNivel[4] = ConvertirPreguntas(datos.niveles.intermedio);
            preguntasPorNivel[7] = ConvertirPreguntas(datos.niveles.avanzado);
        }
        else
        {
    ...
    void CargarPreguntasNivel(int nivelSeleccionado)
    {
        // Asegúrate de que el valor de nivelSeleccionado coincida con las claves (1, 4 o 7)
        if (preguntasPorNivel.ContainsKey(nivelSeleccionado))
        {
            // Clonar la lista para no modificar la original
            preguntas = new List<PreguntaConOpciones>(preguntasPorNivel[nivelSeleccionado]);

            // Mezclar la lista de preguntas
            ShufflePreguntas(preguntas);

            // Limitar a 10 preguntas (si hay más de 10)
            if (preguntas.Count > 10)
                preguntas = preguntas.GetRange(0, 10);

            // Mezclar las opciones de cada pregunta
            foreach (var pregunta in preguntas)
            {
                ShufflePreguntaOpciones(pregunta);
            }
    ...
    // Método para mezclar la lista de preguntas
    void ShufflePreguntas(List<PreguntaConOpciones> lista)
    {
    ...
    // Método para mezclar las opciones de una pregunta y actualizar el índice correcto
    void ShufflePreguntaOpciones(PreguntaConOpciones pregunta)
    {
        // Convertir el arreglo de opciones en una lista de pares (opción, índice original)
        List<(string opcion, int indiceOriginal)> listaOpciones = new List<(string, int)>();
        for (int i = 0; i < pregunta.opciones.Length; i++)
        {
            listaOpciones.Add((pregunta.opciones[i], i));
        }

        // Algoritmo Fisher-Yates para mezclar
        for (int i = listaOpciones.Count - 1; i > 0; i--)
        {
            int j = Random.Range(0, i + 1);
            var temp = listaOpciones[i];
            listaOpciones[i] = listaOpciones[j];
            listaOpciones[j] = temp;
        }

        // Reconstruir el arreglo y encontrar la nueva posición de la respuesta correcta
        string[] nuevasOpciones = new string[listaOpciones.Count];
        int nuevoIndiceCorrecto = -1;
        for (int i = 0; i < listaOpciones.Count; i++)
        {
            nuevasOpciones[i] = listaOpciones[i].opcion;
            if (listaOpciones[i].indiceOriginal == pregunta.indiceCorrecto)
            {
                nuevoIndiceCorrecto = i;
            }
        }
        pregunta.opciones = nuevasOpciones;
        pregunta.indiceCorrecto = nuevoIndiceCorrecto;
    }


    List<PreguntaConOpciones> ConvertirPreguntas(Pregunta[] preguntasJSON)
    {
        List<PreguntaConOpciones> lista = new List<PreguntaConOpciones>();
        foreach (Pregunta p in preguntasJSON)
        {
            lista.Add(new PreguntaConOpciones(
                p.textoPregunta,
                p.opcionesRespuesta,
                p.indiceRespuestaCorrecta
            ));
        }
        return lista;
    }
